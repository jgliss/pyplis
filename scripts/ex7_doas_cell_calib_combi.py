...
if reload:
    ...
    x = np.arange(10,1334, 1)
    coeff1 =[]
    coeff2=[]
    for idx in x:
        poly, tau_arr, so2_arr = cell.get_calibration_polynomial("aa", idx, row, radius_abs=5)
        coeff1.append(poly.coeffs[0])
        coeff2.append(poly.coeffs[1])
        

fig, ax  =plt.subplots(1,3, figsize=(18,6))   
for k in range(len(masks)):
    mask = masks[k]
    ax[0].plot(mask[row,:], label = "CD: %.2e" %cds[k])
ax[1].plot(coeff1)
