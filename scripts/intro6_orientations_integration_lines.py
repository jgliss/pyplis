...
lines_r.append(LineOnImage(0, 0, 1, 1, normal_orientation="right"))

# Slanted line 45 degrees
lines_l.append(LineOnImage(0, 0, 1, 1, normal_orientation="left"))
...
for k in range(len(lines_r)):
    ...
    lbl = "%d: n=%s, n_a= %.1f" %(k, line.normal_vector, line.normal_theta)
    line.plot_line_on_grid(ax = ax[0], label = lbl)
for k in range(len(lines_l)):
    line = lines_l[k]
    lbl = "%d: n=%s, n_a= %.1f" %(k, line.normal_vector, line.normal_theta)
    line.plot_line_on_grid(ax = ax[1], label = lbl)
