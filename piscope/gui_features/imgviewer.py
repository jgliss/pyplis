...
from .plot_widgets import DoubleGraphCanvasMain
from .edit_widgets import DispImagePrepSettings
from .object_widgets import AllFormsPopup
...
class ImgViewer(QWidget):
    ...
    def __init__(self, img_list = None, parent = None):
        """Initialise image viewer
        
        :param str id: Display ID of this viewer (can be different from id of list displayed)
        :param BaseImgList img_list: the image list object to be displayed
        ...
        self.icon_names_menu =  {"prevIm"        :   "Arrow3 Left", 
                               "nextIm"        :   "Arrow3 Right", 
                               "zoomIn"        :   "Zoom In",
                               "zoomOut"       :   "Zoom Out",
                               "fit"           :   "Full Size",
        ...
        self.icon_names_modes = {"drawMode"      :   "Write2",
                                    "editMode"      :   "Tool",
                                    "optFlow"       :   "myFlow",
                                    "tau"           :   "tau"}  
...
#==============================================================================
#         
#         self.img_list.import_filelist(fileList)
#         self.disp_list()
#==============================================================================
...
#==============================================================================
    ...
    def load_all_icons(self):
        ...
        allGood=1
        for key,val in self.icon_names_menu.iteritems():
            try:
        ...
        for key, val in self.icon_names_modes.iteritems():
            try:
        ...
        for key,val in self.buttonIcons.iteritems():
            if key in self.buttons.keys():
                try:
                    ...
                    self.buttons[key].setIconSize(self.icon_size)
...
#==============================================================================
        ...
        if isinstance(lst.current_img(), Img):
            self.histoCanvas.init_histogram(self.img_list.loadedImages["this"])
        self.editCurrentFileNum.setText(str(self.img_list.currentFileNum+1))
        self.check_state()
    
    def check_state(self):
        """Check current state of image list"""
        print ("Checking state of image list:\nTauMode: " + str(self.img_list.tauMode)
         + "\nOptFlowMode: " + str(self.img_list.optFlowEdit.active))
        self.update_tau_mode(self.img_list.tauMode)
        self.update_optflow_mode(self.img_list.optFlowEdit.active)
    ...
    def connect_img_viewer(self, widget):
        ...
        """
        id=widget.img_list.id
        if id not in self.img_list.linkedLists.keys():
            self.img_list.link_imglist(widget.img_list)
    ...
    def disconnect_img_viewer(self,widget):
        ...
        id=widget.img_list.id
        self.img_list.disconnect_linked_imglist(id)
...
#==============================================================================
    ...
    def show_current_edit(self):
        dial=DispImagePrepSettings(self.img_list.imgPrep, parent = self)
        dial.exec_()
    ...
    def disp_list(self):
        """If the img_list object contains files, load the one corresponding to the 
        current file number (see ImgList object) and displays it.
        """
        if self.img_list.files:
            #flag as active
            self.active = True 
            self.disp_image("this") #the currentImg variable will be set here 
            self.write_header()
            self.enable_buttons(True)
            if self.img_list.numberOfFiles < 2:
    ...
    def disp_image(self, key):
        ...
        If image (:mod:`piscope.Image.Img` object) is available in img_list, 
        ...
        """
        t0 = time()
        if self.img_list.loadedImages[key] is not None:            
            self.draw_histogram(key)
            self.currentDispIm = self.img_list.loadedImages[key]
...
#==============================================================================
    ...
    def draw_histogram(self,key):
        ...
        self.histoCanvas.draw_histogram(self.img_list.loadedImages[key])
...
#==============================================================================
    ...
    def write_header(self):
        self.subHeaderLabel.setText(self.img_list.make_header())
    """Button handles definitions
    """
    def goto_num(self):
        num = int(self.editCurrentFileNum.text()) - 1
        if num < 0 or num > self.img_list.numberOfFiles - 1:
            QMessageBox.warning(self, "Error",\
                "Chosen number out of range, retry...", QMessageBox.Ok)
            return
        self.img_list.goto_im(num)
        self.write_header()
#==============================================================================
#         self.selectButton.setChecked(bool(
#             self.img_list.selectedFilesIndex[self.img_list.currentFileNum]))
#==============================================================================
        #self.create_8bit_images()
        self.disp_image("this")
        self.imgLabel.update_flow(self.img_list.optFlowEdit.flowLinesInt)
        for key, widget in self.connectedWidgets.iteritems():
            widget.disp_image("this")
            widget.write_header()
            widget.imgLabel.update_flow(widget.img_list.optFlowEdit.flowLinesInt)
                    
    def prev(self):
        self.disp_image("prev")
        self.img_list.prev_im()
        self.write_header()
#==============================================================================
#         self.selectButton.setChecked(bool(self.img_list.selectedFilesIndex[
#             self.img_list.currentFileNum]))
#==============================================================================
        self.imgLabel.update_flow(self.img_list.optFlowEdit.flowLinesInt)        
        for key, widget in self.connectedWidgets.iteritems():
            widget.disp_image("this")
    ...
    def next(self):
        ...
        `self.loadedImages` and then reload the images in the `img_list` object 
        calling the function :func:`next_im` in the list. 
        """
        self.disp_image("next")
        self.img_list.next_im()
        self.write_header()
        self.imgLabel.update_flow(self.img_list.optFlowEdit.flowLinesInt)
        for key, widget in self.connectedWidgets.iteritems():
            widget.disp_image("this")
...
#             self.img_list.selectedFilesIndex[self.img_list.currentFileNum]=1
#         else:
#             self.img_list.selectedFilesIndex[self.img_list.currentFileNum]=0
#         num=str(int(sum(self.img_list.selectedFilesIndex)))
...
#         if any(a for a in self.img_list.selectedFilesIndex):
...
#                 self.reload_list(self.img_list.selectedFilesIndex)
#                 self.reloadSelectionButton.setEnabled(False)
#             else:
...
#==============================================================================
    ...
    def change_tau_mode(self):
        newMode = not self.img_list.tauMode
        try:
    ...
    def update_tau_mode(self, newMode):
        ...
        self.img_list.activate_tau_mode(newMode)
        if self.buttonMode == "icons":
        ...
        self.histoCanvas.init_histogram(self.img_list.current_img())
    ...
    def change_optflow_mode(self):
        newMode = not self.img_list.optFlowEdit.active
    ...
    def update_optflow_mode(self,newMode):
        ...
        self.img_list.optFlowEdit.active = newMode
        self.img_list.set_flow_images()
        self.img_list.optFlowEdit.calc_flow()
        self.img_list.optFlowEdit.calc_flow_lines()
...
#     def init_edit(self):
#         """Reset image edit"""
#         self.img_list.init_edit()
#         self.disp_image("this")
#         
#==============================================================================
    def reload_list(self, boolList):
        newList=[d for (d, remove) in\
            zip(self.img_list.files, boolList) if remove]
        self.img_list.import_filelist(newList)
        self.img_list.load_images()
    ...
    def _write_mouse_position(self):
        if self.active and self.img_list.loadedImages["this"].img.ndim==2:
            x, y = self.imgLabel.map(self.imgLabel.mousePosX,\
                                                self.imgLabel.mousePosY)
            z = self.formatIntensity[self.img_list.tauMode](self.img_list.\
